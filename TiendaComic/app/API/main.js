// Función que maneja la lógica para cargar un personaje y sus cómics asociados.
function character() {
  ...
  // Función que realiza la conexión para obtener información sobre el personaje.
  function connection() {
  ...
  // Función que obtiene y muestra información sobre los cómics asociados a un personaje.
  ...
  // Función que se llama cuando se carga una página para mostrar información sobre un cómic específico.
  ...
  // Función que muestra información sobre un creador de cómics y los cómics asociados.
  ...
  // Función que muestra información sobre un cómic específico y los creadores asociados.
  ...
  // Función que agrega un cómic a la lista de lectura.
  function addToReadingList(comicID) {
    const xhr = new XMLHttpRequest();
    xhr.open('GET', '../controllers/HomeController.php?comic-id=' + comicID, true);
    xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest");

    xhr.onload = function() {
        if (this.status == 200) {
            // Manejar la respuesta del servidor (puede ser un mensaje de éxito o algún otro indicador)
            alert('Comic añadido a la lista de lectura');
        } else {
            alert('Error al añadir el cómic a la lista de lectura');
        }
    };
